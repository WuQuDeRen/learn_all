...
public class DateUtil {
    ...
    public static Date calFutureDate(int days) {
        return DateUtil.calFutureDate(DateUtil.getToday(), days);
    }
    ...
    public static int differentDaysIgnoreHour(Date start, Date end) {
        Date _start = DateUtil.resetDateTime(start, 0, 0, 0);
        Date _end = DateUtil.resetDateTime(end, 0, 0, 0);
    ...
    public static List<Date> getOrderDateList(Date start, Date end) {
        ...
        Date tempDate = start;
        l.add(start);
        int j = DateUtil.differentDaysIgnoreHour(start, end);
        for (int i = 0; i < j; i++) {
            tempDate = DateUtil.calFutureDate(tempDate, 1);
            l.add(tempDate);
        }
        return l;
    }
